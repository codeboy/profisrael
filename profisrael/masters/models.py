...
class Master(models.Model):
    ...
    service_name = models.CharField(max_length=255, verbose_name='Название услуги', null=True, blank=True) 
    pricemin = models.DecimalField(max_digits=5, decimal_places=0, verbose_name='Цена от', null=True, blank=True)
    pricemax = models.DecimalField(max_digits=5, decimal_places=0, verbose_name='Цена до', null=True, blank=True)
    address = models.CharField(max_length=255, verbose_name='Адрес', blank=True) # Когда поле может быть пустым
    phone = models.CharField(max_length=20, verbose_name='Телефон', null=True, blank=False) # Когда поле обязательно к заполнению
    ...
    gallery = models.ImageField(upload_to='media/gallery', verbose_name='Фото ваших работ') # Поле multi pics
